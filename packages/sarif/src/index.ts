    ...
    runs: [
      {
        tool: {
          driver: {
            ...
            rules: result.findings.map((f) => ({ id: f.engineSources[0]?.engineRuleId ?? f.findingId, name: f.title, shortDescription: { text: f.title } })),
          },
        },
        results: result.findings.map((f) => ({
          ruleId: f.engineSources[0]?.engineRuleId ?? f.findingId,
